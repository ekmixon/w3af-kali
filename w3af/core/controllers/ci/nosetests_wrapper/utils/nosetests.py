...
def open_nosetests_output(suffix, first, last):
    name = f'{NOSE_OUTPUT_PREFIX}_{first}-{last}.{suffix}'
...
def run_nosetests(nose_cmd, first, last):
    ...
    """
    logging.debug(f'Called run_nosetests for {get_run_id(first, last)}')
    ...
    while p.poll() is None:
        ...
        if idle_time > NOSE_TIMEOUT:
            ...
            if console.strip().split('\n')[-1].startswith('OK') and 'Ran ' in console:
                ...
                logging.debug(f'Process {get_run_id(first, last)} killed')
    ...
    logging.debug(f'Cleanup for {get_run_id(first, last)}')
